...
export default function Table({ head, body, searchable }) {
    ...
    if (!body || body.length === 0) {
        return (
            <div className="p-4 rounded bg-yellow text-yellow-700 text-sm">Gösterilcek veri bulunmuyor</div>
        )
    }

    if (isMobile) {

        return <TableMobile head={head} body={body} />
                                    ...
                                    {h.sortable &&
                                        (
                                            <button onClick={() => {
                                                ...
                                                {sorting?.key == key && (sorting.orderBy == 'asc' ? <FaSortUp size={14} /> : <FaSortUp size={14} />
                                                )}
